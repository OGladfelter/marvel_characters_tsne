...
d3.csv("data/team_data.csv", function(data) {
    ...
    document.getElementById("teamSelector").onchange = function(){
        ...
        circles.filter(function(d){return !member_ids.includes(Number(d.pic_id))}).style('opacity', 0.05);
        // and disable tooltip
        ...
        circles.filter(function(d){return member_ids.includes(Number(d.pic_id))}).style('visibility','visible').raise();
    };
