...
if (screen.width < 600){
    ...
    document.getElementById("graph_title").innerHTML = "Use one finger to drag the map, use two fingers to zoom in/out";
}
else{
...
d3.csv("data/Marvel_Character_Data.csv", function(data) {
  ...
  data.forEach(function(d, i) {
    defs.append("svg:pattern")
      ...
      .append("svg:image")
      .attr("xlink:href", "img/" + d.pic_id + ".jpg")
  ...
  var circles = scatter
    ...
    .append("circle")
      ...
      .style("opacity", 1)
      .style("fill", "#fff")
      .style("visibility", function(d) { if (d.pic_id <= document.getElementById("myRange").value) return "visible"; else return "hidden"})
  ...
  document.getElementById("searchButton").onclick = function(){
    ...
    // override hidden visibility or lowered opacity from other interactives
    d3.select("#id_" + search_results[0].pic_id).style('visibility', 'visible').style('opacity', 1).attr('pointer-events', 'auto');
